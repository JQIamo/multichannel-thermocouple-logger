...
// Enables connections to influxDB server (warning: enabling influxDB without properly configuring influxDB settings will cause the data logger to be unresponsive)
boolean enableInfluxDB = ENABLEINFLUX;
...
void setup() {
 ...
 //Initializes SPI
  ...
  // 
  timer.setInterval(5000, influxDBTemp);
}
...
void loop() {
 ...
 if (connectionAttempt > maxConnections){
    resetClient();  
 }
...
void ethernetTemp() {
  ...
  if (client) {
    ...
    Serial.print("Client not available (connection attempt: ");
...
void influxDBTemp(){
  String tempData = "thermo,channel=";
  int i = 0;
  int cTemp;
  int fTemp;
  int currentChanNumber;
  ...
  EthernetClient client;
   ...
   while (i < 20 ) {
    ...
    if (cTemp != 0 && fTemp != 0){
      ...
      Serial.print("Error: Could not connect to influxDB server. ");
      errorActive = true;
      errorNumber = 0;
